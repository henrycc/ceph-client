...
#define RBD_MAX_SNAP_NAME_LEN	NAME_MAX
...
static int rbd_init_disk(struct rbd_device *rbd_dev, u64 disk_size)
{
	struct gendisk *disk;
	struct request_queue *q;
	int rc;
	...
	pr_info("%s: added with size 0x%llx\n",
		disk->disk_name, (unsigned long long) disk_size);
	return 0;
...
static int rbd_dev_v1_snapc_features(struct rbd_device *rbd_dev, u32 which,
		u64 *snap_features)
{
	BUG_ON(which >= rbd_dev->header.snapc->num_snaps);
...
static int __rbd_init_snaps_header(struct rbd_device *rbd_dev)
{
	...
	while (index < snap_count || links != head) {
		...
		if (ret < 0)
			return ret;
		ret = rbd_dev_v1_snapc_features(rbd_dev, index, &snap_features);
		if (ret < 0)
			return ret;
		...
		if (!snap || (snap_id != CEPH_NOSNAP && snap->id < snap_id)) {
			...
			BUG_ON(snap->features != snap_features);
 ...
 * image.
 */
static int __rbd_dev_v2_snapc_size(struct rbd_device *rbd_dev, u64 snap_id,
...
static int __rbd_dev_v2_snapc_features(struct rbd_device *rbd_dev, u64 snap_id,
		u64 *snap_features)
{
	__le64 snapid = cpu_to_le64(snap_id);
	__le64 features = 0;
...
static char *rbd_dev_v2_snapc_name(struct rbd_device *rbd_dev, u32 which)
{
	size_t size;
	void *reply_buf;
	struct ceph_snap_context *snapc;
	...
	if (ret < 0)
		goto out;
	dout("  rbd_req_sync_exec(name[%u]) -> %d\n",
...
static int rbd_dev_v2_snap_context(struct rbd_device *rbd_dev)
{
	...
	ceph_decode_64_safe(&p, end, seq, out_err);
	ceph_decode_32_safe(&p, end, snap_count, out_err);
	...
	snapc->seq = seq;
	snapc->num_snaps = 0;		/* Updated below */
	for (i = 0; i < snap_count; i++)
		snapc->snaps[i] = ceph_decode_64(&p);

	/* Get the other snapshot information (name, size, features) */

	for (i = 0; i < snap_count; i++) {
		ret = rbd_dev_v2_snap_info(rbd_dev, i);
		if (ret < 0)
			goto err_snapc;
		snapc->num_snaps++;
	}
...
{
	return __rbd_dev_v2_snapc_features(rbd_dev, CEPH_NOSNAP,
